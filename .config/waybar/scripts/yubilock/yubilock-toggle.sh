...
SCRIPT_PATH="$HOME/.config/waybar/scripts/yubilock/yubilock.sh"
...
if [ "$current_state" = "on" ]; then
  ...
  echo '{"text": "", "class": "yubilock-off", "tooltip": "Yubilock disabled"}'
else
  # Turn on monitoring
  echo "on" >"$STATE_FILE"
  ...
  "$SCRIPT_PATH" &
  echo '{"text": "", "class": "yubilock-on", "tooltip": "Yubilock enabled"}'
fi
